...
`Settings` reads up to `AI_LINE_LIMIT` ordered failover lines (8) — `AI_1_PROVIDER` / `AI_1_API_KEY` / `AI_1_MODEL` through `AI_8_*` — and `ai_chain` turns them into a list of frozen `AiLine(provider, api_key, model)`. The fields are generated by `create_model` from `AI_LINES`, so the count lives in exactly one constant; keep new per-line validators registered through `_line_fields()` rather than naming fields literally. Providers are `gemini`, `groq`, and `openai`. Points that are easy to get wrong:
...
- **`extra="ignore"` means a typo'd or out-of-range line name vanishes silently.** `AI_1_PROVIER=groq` does not raise; it disables the line, and so does an `AI_9_*` line above the limit. This is what broke the chain once already: `.env` moved to `AI_1_*` while `Settings` still only knew `AI_PROVIDER`, so every line was dropped and `ai_configured` went quietly `False`.
