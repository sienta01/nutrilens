...
- **Share meals** is a separate opt-in for your meal names, meal times, nutrition, and photos from the last 7 days. The community page shows one day per person at a time — today by default; selecting a bar on someone's weekly chart loads that day. Nothing older than 7 days is published, and the limit is fixed rather than a request parameter. Your personal meal notes, email, login details, and Telegram identity stay private. Turning progress sharing off also turns meal sharing off. Previously shared photo URLs check permissions on every request.
